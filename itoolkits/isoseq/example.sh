...
# INPUT: the full isoseq dataset using the flea chemistry
full_ds=/pbi/collections/315/3150353/r54086_20160831_010819/4_D01/m54086_160831_230338.subreads.bam
# INPUT: the existing isoseq job directory of the full dataset
full_ds_isoseq_job_dir=/pbi/dept/secondary/siv/smrtlink/smrtlink-alpha/smrtsuite_170220/userdata/jobs_root/000/000626/tasks/pbtranscript.tasks.combine_cluster_bins-0
...
function run_isoseq()
{
    ...
    if [ -f $ds_xml ]; then
        echo 'Found tiny flea dataset at ' $ds_xml
...
function usage()
{
    echo "Usage: $0 make_tiny_ds # to make a tiny flea dataset"
    echo "       $0 run_isoseq   # to run isoseq on the tiny flea dataset"
    echo "       $0 run_isoseq_w_genome   # to run isoseq within SIRV genome on the tiny flea dataset"
}
