...
def get_parser():
    ...
    p.add_argument("in_zmws", help="Input txt file containg ZMW names or read names")
...
def run(args):
    ...
    for ext in exts:
        if out_prefix.endswith(ext):
            ...
            break
    make_subreads_bam_of_zmws_in_fasta(in_fasta=args.in_fasta,
            in_bam_fofn=args.in_bam_fofn, out_prefix=out_prefix, dry_run=args.dry_run)
    return 0
