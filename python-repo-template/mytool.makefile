...
doctest:
	py.test --doctest-modules newtool/
pylint:
	pylint --errors-only newtool/
wheel:
...
bamboo:
	bash bamboo_newtool_build.sh
	bamboo_build_working_directory=$(shell pwd) bash bamboo_newtool_test.sh
build:
